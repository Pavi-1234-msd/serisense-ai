...
function SilkwormDisease() {
  ...
  // WhatsApp Share Function for Silkworm Health Assessment
  const handleWhatsAppShareSilkworm = () => {
    if (!activeDiagnosis) return;

    const diseaseName = activeDiagnosis.disease || activeDiagnosis.top_disease || 'Suspected Silkworm Pathology';
    const matchPct = activeDiagnosis.match_percentage || activeDiagnosis.match_percent || 0;
    const category = activeDiagnosis.category || activeDiagnosis.type || 'Pathological Infection';
    const mortality = activeDiagnosis.mortality_risk || activeDiagnosis.mortality || 'High';

    const message =
`🐛 *SeriSense AI — Silkworm Health Assessment Report*
━━━━━━━━━━━━━━━━━━━━━━
📋 *Primary Assessment:* ${diseaseName}
🔬 *Pathogen Category:* ${category}
📊 *Symptom Match:* ${matchPct}%
💀 *Mortality Risk Level:* ${mortality}
━━━━━━━━━━━━━━━━━━━━━━
📝 *Observed Characteristics:*
${activeDiagnosis.description || 'Clinical symptom pattern detected during rearing-bed assessment.'}

🛠️ *Immediate Farm Actions:*
${activeDiagnosis.treatment || 'Segregate affected rearing trays immediately.'}

🛡️ *Batch Prevention & Disinfection SOP:*
${Array.isArray(activeDiagnosis.prevention)
  ? activeDiagnosis.prevention.slice(0, 3).map((p, i) => `${i + 1}. ${p}`).join('\n')
  : (activeDiagnosis.prevention || 'Maintain rearing hygiene')}
━━━━━━━━━━━━━━━━━━━━━━
          ...
          {activeDiagnosis && !loading && (
            <div className="diagnosis-report">
              ...
              <div className="whatsapp-share-bar" style={{ margin: '14px 0 10px' }}>
                <button
                  type="button"
                  onClick={handleWhatsAppShareSilkworm}
                  style={{
                    width: '100%',
                    padding: '12px 18px',
                    borderRadius: '12px',
                ...
                >
                  ...
                  {t('btn_share_whatsapp') || 'Share Assessment on WhatsApp'}
                </button>
              </div>
